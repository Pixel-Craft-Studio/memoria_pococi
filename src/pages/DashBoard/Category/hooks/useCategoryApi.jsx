...
export const useCategoryApi = (setIsModalOpen, setFormData, emptyFormData) => {
  ...
  const {
    ...
    isLoading: isPosting,
    error: postError,
  } = usePost(ENDPOINTS.TIMELINE_CATEGORY);
  ...
  const {
    ...
    error: patchError,
  } = usePatch(ENDPOINTS.TIMELINE_CATEGORY);
  ...
  const {
    ...
    error: deleteError,
  } = useDelete(ENDPOINTS.TIMELINE_CATEGORY);
  ...
  useEffect(() => {
    ...
    handleSuccess("Se ha registrado la categoria exitosamente");
  }, [createDataResponse, handleSuccess]);

  useEffect(() => {
    if (!updateDataResponse) return;
    handleSuccess("Se ha actualizado la categoria exitosamente");
  }, [handleSuccess, updateDataResponse]);

  useEffect(() => {
    if (!deleteDataResponse) return;
    handleSuccess("Se ha eliminado la categoria exitosamente");
